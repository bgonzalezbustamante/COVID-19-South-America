...
``` r
## Dataframe
data_arg <- select(south_am, dates, ARG)
names(data_arg)[2] = "I"

## Parametric Re
arg_param_si <- estimate_R(data_arg, method="parametric_si",
...
## Plot 2
arg_Re_may <- 
  plot(arg_param_si, "R") + theme_minimal(base_size = 12) + 
  theme(legend.position = "none") +
  theme(panel.grid.minor = element_blank()) +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, color = "black", 
                                   size = 9)) +
  labs(x = NULL, y = "R", title = "Argentina",
...
## Integration
ggdraw() +
  draw_plot(arg_Re_may) +
  draw_plot(arg_Re_full, x = 0.55, y = 0.55, width = .43, height = .43)
```

![](Re-Argentina_files/figure-gfm/unnamed-chunk-25-1.png)<!-- -->
