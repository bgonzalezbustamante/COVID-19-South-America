...
``` r
## Dataframe
data_ury <- select(south_am, dates, URY)
names(data_ury)[2] = "I"

## Parametric Re
ury_param_si <- estimate_R(data_per, method="parametric_si",
                           config = make_config(list(
                             mean_si = 7.5, std_si = 3.4)))

## Plot 1
ury_Re_full <- 
  plot(ury_param_si, "R") + theme_minimal(base_size = 12) + 
...
## Integration
ggdraw() +
  draw_plot(ury_Re_may) +
  draw_plot(ury_Re_full, x = 0.55, y = 0.55, width = .43, height = .43)
```

![](Re-Uruguay_files/figure-gfm/unnamed-chunk-32-1.png)<!-- -->
