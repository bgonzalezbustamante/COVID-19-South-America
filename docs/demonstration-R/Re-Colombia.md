...
``` r
## Datframe
data_col <- select(south_am, dates, COL)
names(data_col)[2] = "I"

## Parametric Re
col_param_si <- estimate_R(data_col, method="parametric_si",
                           config = make_config(list(
                             mean_si = 7.5, std_si = 3.4)))

## Plot 1
col_Re_full <- 
  plot(col_param_si, "R") + theme_minimal(base_size = 12) + 
  ...
  annotate(geom = "text", x = as.Date("2020-05-8"), y = 3 * 1.6, 
           label = "May", size = 2.5)
...
col_Re_may <- 
  ...
  theme(axis.text.x = element_text(angle = 35, hjust = 1, color = "black", 
                                   size = 9)) +
  labs(x = NULL, y = "R", title = "Colombia",
...
## Integration
ggdraw() +
  draw_plot(col_Re_may) +
  draw_plot(col_Re_full, x = 0.55, y = 0.55, width = .43, height = .43)
```

![](Re-Colombia_files/figure-gfm/unnamed-chunk-29-1.png)<!-- -->
