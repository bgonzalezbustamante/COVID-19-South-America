...
``` r
## Dataframe
data_bra <- select(south_am, dates, BRA)
names(data_bra)[2] = "I"

## Parametric Re
bra_param_si <- estimate_R(data_bra, method="parametric_si",
...
bra_Re_full <- 
  ...
  annotate(geom = "text", x = as.Date("2020-05-8"), y = 3 * 1.27, 
           label = "May", size = 2.5)

## Plot 2
bra_Re_may <- 
  plot(bra_param_si, "R") + theme_minimal(base_size = 12) + 
  theme(legend.position = "none") +
  theme(panel.grid.minor = element_blank()) +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, color = "black", 
                                   size = 9)) +
  labs(x = NULL, y = "R", title = "Brazil",
...
## Integration
ggdraw() +
  draw_plot(bra_Re_may) +
  draw_plot(bra_Re_full, x = 0.55, y = 0.55, width = .43, height = .43)
```

![](Re-Brazil_files/figure-gfm/unnamed-chunk-27-1.png)<!-- -->
